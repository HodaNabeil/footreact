...
function HeandleCard() {
  ...
  return (
    <div className="max-w-[1640px] mx-auto p-4 py-12 ">
      <h2 className="   mx-auto  flex  justify-center items-center  font-medium text-[25px]  md:text-[35px] text-[#f97316]">Order Now</h2>
        <div className="grid md:grid-cols-3 gap-6 p-4  relative ">
        {
                profood.map((pro)=> {
                  return(
                    <div key={pro.id} className=" relative  overflow-hidden">
                      <div className=" absolute  top-[80px]  px-[30px] text-white">
                        <h2 className=" sm:text-xl md:text-3xl font-bold  ">{pro.title}</h2>
                        <p p className=" text-sm my-[15px] ">{pro.details}</p>
                        <button className= 'font-medium   border-none bg-white text-[#f97316] '>Order</button>
                      </div>
